function [ref, rem, indexref] = meshrefmanifold(objP, objt, objnormals, refine)
...
%   SNM 2024
    ...
    while 1
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
        % Faces to include must share all three nodes with faces to be
        % refined
        trem = obj.t(nonrefined, :);
        tref = obj.t(refine, :);
        q1 = ismember(trem, tref);
        q2 = sum(q1, 2);
        q3 = find(q2==3); 
        if sum(q3) == 0
            break;
        end
        refine      = unique([refine; nonrefined(q3)], 'stable');
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        k = k + 1;
    end
    ...
    % %%   Find topological neighbors for the entire mesh
    % DT          = triangulation(obj.t, obj.P); 
    % tneighbor   = neighbors(DT, refine);
    % 
    % %%  Define a neigbor sticking out for every border face (it will be divided into two facets)
    % outerneigh = zeros(F2, 1);
    % for m = 1:F2
    %     temp = setdiff(tneighbor(F3+m, :), refine);
    %     outerneigh(m) = temp; % must always exist
    % end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%  Loop over facets F3 and F2 - find three edge indexes (1 2/1 3/2 3)
